...
interface CacheLoaderWriter<K : Any, V : Any, T> : CacheLoader<K, V>, CacheWriter<K, V, T> {
   ...
   * … FOR UPDATE` on the transaction's connection. Like the writes it is non-suspending, so it runs
   * on the caller's blocking transaction; override it to support [MutableFlowStore.get] within a
   * transaction (the default throws). The read should see the transaction's own uncommitted writes
   * and serialise concurrent writers.
   */
